    ...
    async function copyTextToClipboard(text, btn, doneLabel){
      try{
        if(!navigator.clipboard){
          showAlert("Cæpia de texto n“o suportada neste navegador.");
          return;
        }
    ...
    async function copyImageToClipboard(url, btn, defaultLabel){
      try{
        if(!navigator.clipboard || !window.ClipboardItem){
          showAlert("Cæpia de imagem n“o suportada neste navegador.");
          return;
        }
    ...
    function renderCalendarDayDetails(iso, options){
      ...
      calDayDetails.querySelectorAll("[data-cal-item-del]").forEach(btn=>{
        btn.addEventListener("click", async ()=>{
          ...
          if(!id) return;
          const ok = await showConfirm("Excluir este chamado? (Ele serﬂ removido da lista de tarefas e tamb⁄m do calendﬂrio.)");
          if(!ok) return;
      ...
      if(clearBtn){
        clearBtn.addEventListener("click", async ()=>{
          ...
          if(!dayIso) return;
          const ok = await showConfirm(`Limpar todos os registros do dia ${dayIso} no calendﬂrio?\n\nIsso N+O remove as tarefas em aberto, apenas apaga os itens do calendﬂrio deste dia.`);
          if(!ok) return;
...
/***********************
    ...
    function createDescPhaseBlock(idx, data){
      ...
      attentionLabelText.textContent = "Observa˛ßes";
      ...
      attentionNote.placeholder = "Digite as observa˛ßes...";
    ...
    function renderDescBlock(obs, lojaText, taskId){
      ...
      const phaseBtns = canEdit
        ...
        const calendarBtn = (o._idx === lastIdx)
          ? `<button type="button" class="btn small iconBtn" data-task-phase-calendar="${escapeHtml(safeTaskId)}" data-phase-idx="${escapeHtml(String(o._idx))}" title="Calendﬂrio" aria-label="Calendﬂrio">
               <svg class="iconStroke" viewBox="0 0 24 24" aria-hidden="true">
